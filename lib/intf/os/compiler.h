...
// ── NS_NORETURN — marks a function that never returns ────────────────────
#if defined(_MSC_VER)
...
// ── NS_ANSI_COLORS — 1 if ANSI escape sequences work in terminal output ──
// Old Windows consoles do not support them without extra setup.
#if defined(_WIN32)
#define NS_ANSI_COLORS 0
#else
#define NS_ANSI_COLORS 1
#endif

// ── NS_HAS_BUILTIN_OVERFLOW — 1 if __builtin_{add,sub,mul}_overflow exist ─
#if defined(_MSC_VER)
#define NS_HAS_BUILTIN_OVERFLOW 0
#else
#define NS_HAS_BUILTIN_OVERFLOW 1
#endif
