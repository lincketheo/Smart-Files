...
i64
i_pread_some (const i_file *fp, void *dest, const u64 n, const u64 offset, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (dest);
  ASSERT (n > 0);
...
i64
i_pread_all (const i_file *fp, void *dest, const u64 n, const u64 offset, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (dest);
  ASSERT (n > 0);
  ...
  while (nread < n)
    {
      ...
      const ssize_t _nread
          = pread (fp->fd, _dest + nread, n - nread, offset + nread);
...
i64
i_pwrite_some (const i_file *fp, const void *src, const u64 n, const u64 offset, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (src);
  ASSERT (n > 0);
...
err_t
i_pwrite_all (const i_file *fp, const void *src, const u64 n, const u64 offset, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (src);
  ASSERT (n > 0);
  ...
  while (nwrite < n)
    {
      // Do write
      ASSERT (n > nwrite);
      const ssize_t _nwrite
          = pwrite (fp->fd, _src + nwrite, n - nwrite, offset + nwrite);
...
i64
i_writev_some (const i_file *fp, const struct bytes *src, const int iovcnt, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (src);
  ASSERT (iovcnt > 0);
...
err_t
i_writev_all (const i_file *fp, struct bytes *iov, const int iovcnt, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (iov);
  ASSERT (iovcnt > 0);
...
i64
i_readv_some (const i_file *fp, const struct bytes *iov, const int iovcnt, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (iov);
  ASSERT (iovcnt > 0);
...
i64
i_readv_all (const i_file *fp, struct bytes *iov, const int iovcnt, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (iov);
  ASSERT (iovcnt > 0);
...
i64
i_write_some (const i_file *fp, const void *src, const u64 nbytes, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (src);
  ASSERT (nbytes > 0);
...
err_t
i_write_all (const i_file *fp, const void *src, const u64 nbytes, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (src);
  ASSERT (nbytes > 0);
  ...
  if (fcntl (fp->fd, F_PREALLOCATE, &store) == -1)
    {
      ...
      if (fcntl (fp->fd, F_PREALLOCATE, &store) == -1)
        {
          return error_causef (e, ERR_IO, "F_PREALLOCATE: %s",
                               strerror (errno));
        }
    }
