  ...
  if (h == INVALID_HANDLE_VALUE)
    {
      ...
      return error_causef (e, ERR_IO, "open_rw %s: %s", fname,
                           WIN_ERRMSG (buf));
    }
  dest->handle = h;
...
i_open_r (i_file *dest, const char *fname, error *e)
{
  ...
  if (h == INVALID_HANDLE_VALUE)
    {
      char buf[WIN_ERR_BUF];
      return error_causef (e, ERR_IO, "open_r %s: %s", fname,
                           WIN_ERRMSG (buf));
    }
  dest->handle = h;
  DBG_ASSERT (i_file, dest);
...
i_open_w (i_file *dest, const char *fname, error *e)
{
  ...
  if (h == INVALID_HANDLE_VALUE)
    {
      char buf[WIN_ERR_BUF];
      return error_causef (e, ERR_IO, "open_w %s: %s", fname,
                           WIN_ERRMSG (buf));
    }
  dest->handle = h;
  DBG_ASSERT (i_file, dest);
  ...
  while (nread < n)
    {
      ...
      DWORD chunk = 0;
      DWORD want = (DWORD)((n - nread) > 0xFFFFFFFFULL
                               ? 0xFFFFFFFFUL
                               : (n - nread));
  ...
  while (nwrite < n)
    {
      ...
      DWORD chunk = 0;
      DWORD want = (DWORD)((n - nwrite) > 0xFFFFFFFFULL
                               ? 0xFFFFFFFFUL
                               : (n - nwrite));
...
i64
i_writev_some (const i_file *fp, const struct bytes *src, int iovcnt, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (src);
  ASSERT (iovcnt > 0);
  ...
  for (int i = 0; i < iovcnt; i++)
    {
      ...
      while (remain > 0)
        {
          DWORD want = (DWORD)(remain > 0xFFFFFFFFULL
                                   ? 0xFFFFFFFFUL
                                   : remain);
          DWORD nwritten = 0;
...
i64
i_readv_some (const i_file *fp, const struct bytes *iov, int iovcnt, error *e)
{
  DBG_ASSERT (i_file, fp);
  ASSERT (iov);
  ASSERT (iovcnt > 0);
  ...
  for (int i = 0; i < iovcnt; i++)
    {
      DWORD nread = 0;
      if (!ReadFile (fp->handle, iov[i].head, (DWORD)iov[i].len, &nread,
                     NULL))
        {
          DWORD err = GetLastError ();
          if (err == ERROR_HANDLE_EOF)
...
i_read_all (const i_file *fp, void *dest, u64 nbytes, error *e)
{
  ...
  while (nread < nbytes)
    {
      DWORD chunk = 0;
      DWORD want = (DWORD)((nbytes - nread) > 0xFFFFFFFFULL
                               ? 0xFFFFFFFFUL
                               : (nbytes - nread));
...
i_write_all (const i_file *fp, const void *src, u64 nbytes, error *e)
{
  ...
  while (nwrite < nbytes)
    {
      DWORD chunk = 0;
      DWORD want = (DWORD)((nbytes - nwrite) > 0xFFFFFFFFULL
                               ? 0xFFFFFFFFUL
                               : (nbytes - nwrite));
...
i_fallocate (i_file *fp, u64 bytes, error *e)
{
  ...
  if (!SetFilePointerEx (fp->handle, li, NULL, FILE_BEGIN))
    {
      char buf[WIN_ERR_BUF];
      return error_causef (e, ERR_IO, "fallocate (seek): %s",
                           WIN_ERRMSG (buf));
    }
  if (!SetEndOfFile (fp->handle))
    {
...
i_mkstemp (i_file *dest, char *tmpl, error *e)
{
  ...
  if (!GetTempFileNameA (tmp_dir, "ns_", 0, tmp_path))
    {
      char buf[WIN_ERR_BUF];
...
i_mkdir_quiet (const char *name, error *e)
{
  ...
  if (!(attrs & FILE_ATTRIBUTE_DIRECTORY))
    {
      error_causef (e, ERR_IO,
                    "mkdir_quiet: %s exists but is not a directory", name);
      return error_trace (e);
    }
