...
i32
stream_read (struct stream *dest, const u32 size, const u32 n, struct stream *src,
             error *e)
{
  if (dest->ops->push == NULL)
    {
      return error_causef (e, ERR_INVALID_ARGUMENT, "stream is not writable");
    }
  if (src->ops->pull == NULL)
    {
      return error_causef (e, ERR_INVALID_ARGUMENT, "stream is not readable");
    }
...
static i32
stream_ibuf_pull (struct stream *s, void *vctx, void *dest, const u32 size, const u32 n,
                  error *e)
{
...
static i32
stream_obuf_push (struct stream *s, void *vctx, const void *src, const u32 size,
                  const u32 n, error *e)
{
...
static i32
stream_sink_push (struct stream *s, void *vctx, const void *src, u32 size,
                  const u32 n, error *e)
{
  return n;
}
...
static i32
stream_opsink_push (struct stream *s, void *vctx, const void *src, const u32 size,
                    const u32 n, error *e)
{
...
static i32
stream_limit_pull (struct stream *s, void *vctx, void *buf, const u32 size, const u32 n,
                   error *e)
{
  struct stream_limit_ctx *ctx = (struct stream_limit_ctx *)vctx;

  const b_size remaining = ctx->limit - ctx->consumed;
  if (remaining == 0)
    {
      stream_finish (s);
      return 0;
  ...
  const u32 capped_n = (u32)(MIN ((b_size)(size * n), remaining) / size);
  if (capped_n == 0)
    {
      stream_finish (s);
      return 0;
...
static i32
stream_limit_push (struct stream *s, void *vctx, const void *buf, const u32 size,
                   const u32 n, error *e)
{
  struct stream_limit_ctx *ctx = (struct stream_limit_ctx *)vctx;
...
stream_limit_init (struct stream *s, struct stream_limit_ctx *ctx,
                   struct stream *underlying, const b_size limit)
{
