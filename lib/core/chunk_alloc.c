...
DEFINE_DBG_ASSERT (struct chunk_alloc, chunk_alloc, ca, {
  ...
  ASSERT (ca->settings.max_chunk_size == 0
  ...
  ASSERT (ca->settings.max_total_size == 0
          || ca->total_allocated <= ca->settings.max_total_size);
  ASSERT (ca->settings.max_chunks == 0
  ...
  for (const struct chunk *c = ca->head; c != NULL; c = c->next)
    {
      ...
      ASSERT (counted_chunks <= ca->settings.max_chunks
...
chunk_alloc_add_new_chunk (struct chunk_alloc *ca, const u32 size, error *e)
{
  ...
  ASSERT (ca->settings.max_chunk_size == 0
          || size <= ca->settings.max_chunk_size);
...
void *
chunk_alloc_move_mem (struct chunk_alloc *ca, const void *ptr, const u32 size,
                      error *e)
{
