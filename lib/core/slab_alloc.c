...
TEST (TT_UNIT, slab_alloc_cap_one)
{
  struct slab_alloc alloc;
  error e = error_create ();
...
TEST (TT_UNIT, slab_alloc_no_duplicates)
{
  struct slab_alloc alloc;
  error e = error_create ();
  const int N = 100;
...
TEST (TT_UNIT, slab_alloc_free_all_realloc)
{
  struct slab_alloc alloc;
  error e = error_create ();
...
TEST (TT_UNIT, slab_alloc_interleaved_patterns)
{
  struct slab_alloc alloc;
  error e = error_create ();
...
TEST (TT_UNIT, slab_alloc_free_head_slab)
{
  struct slab_alloc alloc;
  error e = error_create ();
...
TEST (TT_UNIT, slab_alloc_free_middle_slab)
{
  struct slab_alloc alloc;
  error e = error_create ();
...
TEST (TT_UNIT, slab_alloc_minimum_size)
{
  struct slab_alloc alloc;
  error e = error_create ();
...
TEST (TT_UNIT, slab_alloc_stress_random)
{
  struct slab_alloc alloc;
  error e = error_create ();
  const int POOL = 256;
