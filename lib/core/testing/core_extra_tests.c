...
TEST (TT_UNIT, f16_to_f32_normals_and_specials)
{
...
TEST (TT_UNIT, f16_to_f32_nan_is_nan)
{
...
// The current code emits (mant<<13) as an f32 subnormal, giving ~2^-136 —
// wrong. KNOWN BUG: this test currently FAILS — disabled until fixed.
TEST (TT_UNIT, f16_to_f32_smallest_subnormal_correct_value)
{
...
TEST (TT_UNIT, parse_i32_boundary_values)
{
...
TEST (TT_UNIT, parse_i64_boundary_values)
{
  error e = error_create ();
  i64 out = 0;
...
TEST (TT_UNIT, ext_array_capacity_doubles_on_growth)
{
  error e = error_create ();
...
TEST (TT_UNIT, ext_array_remove_all_produces_empty)
{
  error e = error_create ();
  struct ext_array a = ext_array_create ();
...
TEST (TT_UNIT, llist_append_maintains_fifo_order)
{
  ...
  for (int i = 0; i < 5; ++i)
    {
      ...
      test_assert (n != NULL);
      test_assert_int_equal (container_of (n, struct llt_node, link)->value,
                             i);
    }
...
TEST (TT_UNIT, llist_find_returns_node_and_index)
{
...
TEST (TT_UNIT, llist_remove_from_head_middle_tail)
{
...
TEST (TT_UNIT, llist_remove_absent_node_is_noop)
{
...
TEST (TT_UNIT, checksum_known_crc32c_vector)
{
...
TEST (TT_UNIT, checksum_distinct_bytes_differ)
{
  const u8 a[] = { 0x00 };
  const u8 b[] = { 0x01 };
...
// false.
TEST (TT_UNIT, serializer_write_at_capacity_then_overflow)
{
  u8 buf[8];
...
TEST (TT_UNIT, serializer_incremental_write_overflow)
{
  u8 buf[6];
  struct serializer s = srlizr_create (buf, sizeof buf);
...
TEST (TT_UNIT, stride_constructors_resolve_correctly)
{
  struct stride r;
  error e = error_create ();
  const b_size N = 10;
  ...
  TEST_CASE ("usfrms round-trip: stride → user_stride → stride is identity")
  {
    ...
    test_assert_type_equal (got.start, orig.start, b_size, PRb_size);
    test_assert_type_equal (got.stride, orig.stride, u32, PRIu32);
    test_assert_type_equal (got.nelems, orig.nelems, b_size, PRb_size);
  }
}

TEST (TT_UNIT, string_ordering_operators)
{
...
TEST (TT_UNIT, line_length_newline_found)
{
...
TEST (TT_UNIT, string_equal_cases)
{
...
TEST (TT_UNIT, strings_are_disjoint_cases)
{
  ...
  TEST_CASE ("multiple right candidates, none match: returns NULL")
  {
    ...
    const struct string right[]
        = { { .len = 1, .data = db }, { .len = 1, .data = dc } };
    test_assert (strings_are_disjoint (left, 1, right, 2) == NULL);
  }
}

TEST (TT_UNIT, string_plus_concatenates)
{
...
TEST (TT_UNIT, cbuffer_discard_all_resets_state)
{
...
TEST (TT_UNIT, cbuffer_read_write_wraparound)
{
...
TEST (TT_UNIT, cbuffer_cbuffer_move_transfers_bytes)
{
