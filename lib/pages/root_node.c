...
rn_validate_for_db (const page *p, error *e)
{
  if (page_get_type (p) != PG_ROOT_NODE)
    {
      return error_causef (e, ERR_CORRUPT, "wrong page type for root node");
    }
  return SUCCESS;
}

#ifndef NTEST
TEST (rn_validate_for_db)
{
...
void
i_log_rn (const int level, const page *rn)
{
