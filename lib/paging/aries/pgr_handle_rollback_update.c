...
static err_t
append_and_flush_clr (struct pager *p, const struct wal_clr_write clr,
                      struct txn *tx, page_h *ph, lsn *undo_nxt_lsn,
                      const lsn prev_lsn, error *e)
{
  const slsn clr_lsn = oswal_append_clr_log (p->ww, clr, e);
  if (clr_lsn < 0)
    {
...
static err_t
pgr_handle_physical_rollback_update (struct pager *p,
                                     ...
                                     lsn *undo_nxt_lsn, error *e)
{
  ...
  return append_and_flush_clr (p,
                               ...
                               tx, ph, undo_nxt_lsn, prev_lsn, e);
}

static err_t
pgr_handle_fsm_rollback_update (struct pager *p,
...
err_t
pgr_handle_rollback_update (struct pager *p, struct wal_rec_hdr_read *log_rec,
                            struct txn *tx, page_h *ph, lsn *undo_nxt_lsn,
                            error *e)
{
  ...
  switch (log_rec->update.type)
    ...
    case WUP_PHYSICAL:
      {
        return pgr_handle_physical_rollback_update (p, log_rec, tx, ph,
                                                    undo_nxt_lsn, e);
      }
    case WUP_FSM:
      {
        return pgr_handle_fsm_rollback_update (p, log_rec, tx, ph,
                                               undo_nxt_lsn, e);
      }
    case WUP_FEXT:
      {
        ...
        // any logs that point to a file extension I
        // think this might be a corrupt bug
        UNREACHABLE ();
      }
    }
  UNREACHABLE ();
